    ...
    private void MainWindowUI(int windowId)
    {
        ...
        { // Main Window Area
            ...
            GUILayout.BeginVertical();
            {
                ...
                GUILayout.BeginHorizontal();
                {
                    GUILayout.Label("Sound: ", GUILayout.Width(WINDOW_DIMENSION.x / 4.0f));
                    var sound = SoundManager.Instance.masterVolume;
                    sound = GUILayout.HorizontalSlider(sound, -80.0f, 20.0f, 
                        GUILayout.ExpandWidth(true));
                    if (GUI.changed)
                    { SoundManager.Instance.masterVolume = sound; }
                    
                    if (GUILayout.Button("Mute", GUILayout.Width(50.0f)))
                    {
